...
function Invoke-Api {
    ...
    if ($Method -ne 'GET') { $request.Headers = @{ $demoCsrf.headerName = $demoCsrf.token } }
    if ($null -ne $Body) {
...
if ($null -eq $Credential) {
    $Credential = Get-Credential -UserName 'operator' -Message 'Sign in to the ClinicFlow demo'
}
if ($null -eq $Credential) { throw 'Sign-in credentials are required.' }
...
$loginBody = @{ username = $Credential.UserName; password = $Credential.GetNetworkCredential().Password }
try {
    $null = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/auth/login" -WebSession $demoSession `
        -ContentType 'application/x-www-form-urlencoded' -Headers @{ $demoCsrf.headerName = $demoCsrf.token } `
...
try {
    ...
    try {
        $demoCsrf = Invoke-RestMethod -Uri "$BaseUrl/api/auth/csrf" -WebSession $demoSession -TimeoutSec 15
        $null = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/auth/logout" -WebSession $demoSession `
            -Headers @{ $demoCsrf.headerName = $demoCsrf.token } -TimeoutSec 15
    } catch {
        Write-Warning 'Demo sign-out could not be confirmed.'
    }
}
